...
import { modulesDbList } from '../../../../mocks/db/certificates.db';
	...
	columnHelper.display({
		cell: (info) => (
			<div className='flex gap-2'>
				{Object.keys(info.row.original.certsRequired).map((m, index) => (
					<Badge
						variant='outline'
						...
						<b>{Object.values(info.row.original.certsRequired)[index]}</b>
					</Badge>
				))}
			</div>
		),
		header: 'Certificates Required',
		footer: 'Certificates Required',
	}),
	columnHelper.display({
		cell: (info) => (
			<Link to={`${editLinkPath}${info.row.original.id}`}>
		...
		<PageWrapper name='Roles List'>
			...
			<Container>
				<Card className='h-full'>
					<CardHeader>
						<CardHeaderChild>
							<CardTitle>Company Rolls</CardTitle>
						</CardHeaderChild>
						<CardHeaderChild>
